            ...
            <div wire:key="foo">
                @foreach($region_list as $region)
                    <div wire:key="region-{{ $region->id }}">
                        <button wire:click.prevent="toggleRegion({{$region->id}})"  class=" @if ($this->region == $region->id) bg-red-200 @endif p-1 rounded-md mb-1" >{{$region->name}}</button>
                    </div>
                @endforeach
            </div>
            ...
            <div wire:key="bar">
                @foreach($category_list as $category)
                    <div wire:key="category-{{ $category->id }}">
                        <button wire:click.prevent="toggleCategory({{$category->id}})" class=" @if ($this->category == $category->id) bg-red-200 @endif p-1 rounded-md mb-1">{{$category->name}}</button>
                    </div>
                @endforeach
            </div>
        </div>
